...
from .code_analyzer import get_code_analyzer, CodeAnalysisResult
...
class CodeIndexer:
    ...
    async def _chunk_file(self, file_path: str, analysis: CodeAnalysisResult) -> List[CodeChunk]:
        """파일을 청크로 분할"""
        chunks = []
    ...
    async def index_directory(self, directory_path: str, max_files: int = None) -> Dict[str, Any]:
        """디렉토리 전체 인덱싱"""
        try:
            ...
            # 지원하는 파일 확장자
            supported_extensions = getattr(settings.project, 'supported_extensions', [
...
class RAGSystem:
    ...
    async def search_codebase(
        ...
        try:
            ...
            logger.info(f"검색 완료: {len(vector_results)} 결과")
