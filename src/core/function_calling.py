...
class FunctionCallRegistry:
    ...
    def register_function(self, func_def: FunctionDefinition):
        ...
        logger.info(f"함수 등록됨: {func_def.name}")
    ...
    async def _search_codebase(self, query: str, max_results: int = 5, file_type: str = None) -> Dict[str, Any]:
        """코드베이스 검색"""
        try:
            # 파일 타입 필터링을 위한 쿼리 조정
            if file_type:
    ...
    async def _index_project(self, project_path: str, max_files: int = 1000) -> Dict[str, Any]:
        """프로젝트 인덱싱"""
        try:
            logger.info(f"프로젝트 인덱싱 시작: {project_path}")
    ...
    async def call_function(self, function_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"함수 실행 완료: {function_name}")
            return result
            
        except Exception as e:
            logger.error(f"함수 실행 실패 {function_name}: {e}")
            return {"status": "error", "message": str(e)}
...
class EnhancedLLMManager:
    ...
    def __init__(self, base_llm_manager):
...
```"""
            ...
            if enable_function_calling:
                ...
                if function_calls:
                    logger.info(f"함수 호출 감지: {len(function_calls)}개")
...
{prompt}"""
            ...
            logger.error(f"Function calling 응답 생성 실패: {e}")
            # 폴백: 기본 LLM 응답
            return await self.base_llm.generate_response(prompt, max_tokens, temperature)
...
def get_function_registry() -> FunctionCallRegistry:
    ...
    async def _analyze_file(self, file_path: str, include_content: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not analysis:
                return {"status": "error", "message": f"파일 분석 실패: {file_path}"}
